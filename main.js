...
camera.position.set(8, 8, isMobile ? 15 : 20);
camera.lookAt(scene.position);
scene.add(camera);

const image = new Image();
image.src = 'public/ChLogoTech.png';

image.onerror = () => {
  console.error('Failed to load image at public/ChLogoTech.png');
...
function setupScene(texture) {
  ...
  function animate(time) {
    ...
    rotationVelocity.x += 0.0001 * (Math.random() - 0.5);
    rotationVelocity.y += 0.0001 * (Math.random() - 0.5);
    rotationVelocity.z += 0.0001 * (Math.random() - 0.5);

    if (time - lastTime >= updateInterval) {
      for (let i = 0; i < count; i++) {
        const offset = Math.sin(time * 0.002 + phases[i]) * 0.15;
